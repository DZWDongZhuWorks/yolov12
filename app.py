    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
                ...
                gr.Markdown("### Polygon 優化（可多步驟套用 convex_hull / rdp / visvalingam_whyatt）")
                ...
                with gr.Accordion("Polygon 套用 Classes（不選=全部）", open=False):
                    polygon_step_class_filter = gr.CheckboxGroup(
                        label="Polygon 套用 Classes（不選=全部）",
                        choices=[],
                        value=[],
                    )
                    with gr.Row():
                ...
                gr.Markdown("可直接編輯下表調整順序、次數、參數與 classes。")
        ...
        def run_inference(
            ...
            if input_type_in == "Image":
                ...
                gallery, results_cache = yolov12_multi_inference_image(
                    ...
                    polygon_opt_enabled=polygon_opt_enable_in,
                    polygon_opt_steps=polygon_opt_steps_in,
                )
                ...
                return (
                    ...
                    gr.update(choices=class_choices_new, value=[]),
                    gr.update(value=""),
        ...
        def step_select_all_classes(choices):
            return gr.update(value=choices or []), gr.update(value=""), (choices or [])
        ...
        def polygon_step_select_all_classes(choices):
            return gr.update(value=choices or []), gr.update(value=""), (choices or [])

        def polygon_step_clear_all_classes():
            return gr.update(value=[]), gr.update(value=""), []

        polygon_step_select_all_btn.click(
            fn=polygon_step_select_all_classes,
            inputs=[class_choices_state],
        ...
        polygon_step_clear_all_btn.click(
            fn=polygon_step_clear_all_classes,
            inputs=[],
            outputs=[polygon_step_class_filter, polygon_step_class_filter_query, polygon_step_selected_classes_global],
        )
