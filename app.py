                    ...
                    with gr.Tab("Polygon 優化"):
                        ...
                        with gr.Row():
                            polygon_opt_method = gr.Dropdown(
                                label="新增步驟",
                                choices=["convex_hull", "rdp", "visvalingam_whyatt", "min_area_rect", "export_line", "trans_lane_line_polygon", "pca"],
                                value="rdp",
                            )
                        ...
                        with gr.Row():
                            ...
                            polygon_step_min_aspect = gr.Slider(
                                label="Min Aspect (min_area_rect / export_line / trans_lane_line_polygon)",
                                minimum=0.0,
                                maximum=20.0,
                                step=0.1,
                                value=0.0,
                        ...
                        gr.Markdown(
                            "可直接編輯下表調整 Polygon 優化順序、次數與參數。`eps_coeff` 用於 rdp/visvalingam、trans_lane_line_polygon 簡化強度或 pca 對齊強度；`min_aspect` 用於 min_area_rect/export_line/trans_lane_line_polygon；`pca_min_cosine` 用於方向分群門檻；`pca_cross_class` 控制是否跨類別共同計算 PCA；`classes` 欄位可捲動（留空 = 全部類別）。"
                        )
